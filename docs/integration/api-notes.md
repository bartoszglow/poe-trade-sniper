...
| `wss://…/api/trade2/live/<realm>/<league>/<id>`              | Push detection. Frames: `{"new": ["<listingId>", …]}`; anything else is keepalive/noise. **GGG backend 504s since ~patch 0.5.0** — probe and fall back to polling; auto-resume when it returns. **Tarpit:** unauthenticated handshakes hang forever — always send session cookies and enforce a connect timeout. | 2026-06-11 |
